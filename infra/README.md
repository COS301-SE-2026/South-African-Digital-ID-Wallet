...
### 5. Seed Key Vault secrets (first deploy only)

After the infrastructure exists, populate the secrets that the apps read at runtime:
...
This prompts you for each secret value and writes them to Key Vault. The apps use `@Microsoft.KeyVault(...)` references to read them — no secrets are ever stored in Bicep.

## Design decisions

- **No secrets in code.** Key Vault uses RBAC auth (`enableRbacAuthorization: true`). SQL password is read from an environment variable at deploy time. All app-level secrets (connection strings, API keys, JWT keys) are stored in Key Vault and referenced via `@Microsoft.KeyVault(VaultName=...;SecretName=...)` in app settings.
- **App settings are a full replace.** Bicep's `siteConfig.appSettings` replaces all settings on each deploy — every setting the app needs is declared in `main.bicep`. This prevents configuration drift but means new settings must be added to the template.
- **App URLs are params, not outputs.** Azure hostnames contain random hashes (e.g. `api-flashid-dev-bjgng2dxd6hrgbca...`). To avoid circular dependencies between apps that reference each other's URLs, hostnames are declared as params in the `.bicepparam` files.
- **Managed identities** (`SystemAssigned`) on all App Services with RBAC role assignments for Key Vault, Storage, and Cognitive Services.
