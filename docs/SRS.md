...
##### R2.3.5:
The system shall allow administrators to place a credential in an Under Investigation status as a temporary measure before a final revocation decision is made. A credential Under Investigation shall return an INVALID result on verification.
...
##### R2.3.7:
The system shall run a scheduled automated job to check credential expiry dates daily. Any credential whose expiry date has passed shall have its status automatically updated to Expired. An expired credential shall return INVALID on verification immediately after the status update.
...
##### R3.1.6:
The system shall display a live expiry countdown timer to the citizen while a QR code is active. A visual warning shall be shown when fewer than 60 seconds remain. When the timer reaches zero the QR code shall be visually replaced with an expired state and the citizen shall be prompted to generate a new QR code.
...
##### R3.2.8:
The system shall not include any citizen personally identifiable information in the verification response. The response shall contain only the result (VALID or INVALID), credential type, failure reason if applicable, and timestamp.
...
##### R3.5.4:
For any field the citizen denies, the system shall return a not_disclosed indicator to the official for that field. No value shall be returned for denied fields.
...
##### R4.1.1:
The system shall restrict users to functionality permitted by their assigned role. The system shall support the following roles: Citizen, Home Affairs Official, Government Administrator, and Emergency Responder.
...
##### R5.1.1:
The system shall log: Login attempts, Credential issuance, Credential revocation, Verification attempts, Permission changes.
...
The FlashID system shall support the registration and management of external institutions and their API access credentials.
...
The FlashID system shall use Ed25519 digital signatures to guarantee the authenticity and integrity of all credentials and QR codes.
...
##### R9.3.1:
The system shall support cryptographic key rotation without invalidating existing valid credentials that were signed with a previous key.
...
The FlashID system shall allow citizens to manage their account security and respond to device loss or coercion.
...
##### R10.2.2:
The system shall require OTP verification of the new contact detail before the change takes effect. The previous contact detail shall remain active until verification succeeds.
