...
Database access is abstracted through repository interfaces defined in the Application layer and implemented in the Infrastructure layer using EF Core. This decouples business logic from SQL Server specifics and makes services independently testable. Most repositories are implemented using EF Core against Azure SQL, but in this system, the QR disclosure-token repository is implemented against Azure Cosmos DB instead, behind the same interface shape, so `QrService` and its consumers are unaware of which underlying store is used.
