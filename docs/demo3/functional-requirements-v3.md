...
#### R2.4.5
Citizen photographs referenced by a credential shall be stored in a location that does not allow public access. A link granting temporary access to a photograph sha;; only be created at the moment a credential is being verified, and that link shall stop working after a short, fixed period of time.
