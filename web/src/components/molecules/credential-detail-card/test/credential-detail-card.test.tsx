...
const renderCard = (onGenerateCertifiedCopy = jest.fn()) =>
  render(
...
describe('CredentialDetailCard', () => {
  ...
  it('renders the credential action buttons', () => {
    renderCard()
    expect(
    ...
    expect(
      screen.getByRole('button', {
        name: /generate certified copy/i,
      })
    ).toBeInTheDocument()
  })
  it('calls onGenerateCertifiedCopy when the button is clicked', async () => {
    const user = userEvent.setup()
    const onGenerateCertifiedCopy = jest.fn()
    renderCard(onGenerateCertifiedCopy)
    await user.click(
      screen.getByRole('button', {
        name: /generate certified copy/i,
      })
