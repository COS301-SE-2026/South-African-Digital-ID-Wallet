...
public class CredentialUpdateRepository : ICredentialUpdateRepository
{
    private readonly AppDbContext _context;
    private static readonly TimeSpan StaleRunningThreshold = TimeSpan.FromHours(1);

    public CredentialUpdateRepository(AppDbContext context)
    {
        _context = context;
    }

    public async Task<List<Citizen>> GetCitizensWithActiveCredentialsPageAsync(Guid afterId, int pageSize, CancellationToken cancellationToken)
    {
        return await _context.Citizens
            .Include(c => c.Credentials.Where(cr => cr.Status == CredentialStatus.Active))
                .ThenInclude(cr => cr.IdentityDocument)
            .Include(c => c.Credentials.Where(cr => cr.Status == CredentialStatus.Active))
                .ThenInclude(cr => cr.DriversLicense)
            .Where(c => c.Credentials.Any(cr => cr.Status == CredentialStatus.Active) && c.Id > afterId)
            .OrderBy(c => c.Id)
            .Take(pageSize)
