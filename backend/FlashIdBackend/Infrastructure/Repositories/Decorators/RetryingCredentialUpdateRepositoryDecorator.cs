...
public class RetryingCredentialUpdateRepositoryDecorator : ICredentialUpdateRepository
{
    private readonly ICredentialUpdateRepository _inner;
    private readonly int _maxAttempts;

    public RetryingCredentialUpdateRepositoryDecorator(ICredentialUpdateRepository inner, int maxAttempts = 3)
    {
        _inner = inner;
        _maxAttempts = maxAttempts;
    }

    public Task<List<Citizen>> GetCitizensWithActiveCredentialsPageAsync(Guid afterId, int pageSize, CancellationToken cancellationToken) => _inner.GetCitizensWithActiveCredentialsPageAsync(afterId, pageSize, cancellationToken);
